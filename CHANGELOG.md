...
- 0c4ea3d: Add `worktree clean` to remove clean worktrees whose commits are merged into the origin default branch.
...
- Opening a worktree in Zed now opens a new window instead of replacing the project in your current Zed window, matching how VS Code and Cursor behave.

## [1.3.2] - 2026-07-14

### Added

- Zed editor support — `zed` is now auto-detected alongside VS Code and Cursor. Open a worktree in Zed with `--editor zed`, or pick it from the interactive prompt when multiple editors are installed.
